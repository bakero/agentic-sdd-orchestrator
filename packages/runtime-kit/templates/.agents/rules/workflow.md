...
Gemini creates the feature branch, documentation folder, document templates, and feature index entry.
...
- Context Pack for Codex Architect;
...
Codex Architect creates `technical_spec.md`, initial `test_plan.md`, Context Pack for Claude, Next Agent Cost Recommendation for Claude, and an Agent Cost Log row.
...
Codex Architect must confirm that the required application scaffold exists or creating it is explicitly approved in scope, target implementation areas exist or can be created within scope, package/build/test/application structure is present where required or approved to create, and implementation does not require missing foundational architecture outside scope.

If readiness fails, Codex must block, document the blocker, add a blocking question for Gemini Product Owner, update `status.md`, and use `NEEDS_HUMAN_DECISION` when a product or scope decision is needed.

State: `TECHNICAL_SPEC_READY`, then `READY_FOR_IMPLEMENTATION` only if the Implementation Readiness Gate passes.

## Phase 7 — Implementation

Claude Implementer reads `technical_spec.md`, `test_plan.md`, and the Context Pack from Codex Architect.
...
Claude implements the feature, creates or updates required tests, runs required checks, and updates `implementation_report.md`, `test_plan.md`, `status.md`, Context Pack for Codex Reviewer, Next Agent Cost Recommendation for Codex Reviewer, and Agent Cost Log row.
...
If approved, Codex updates `technical_review.md`, `status.md`, Context Pack for Gemini Functional Validator, Next Agent Cost Recommendation and Agent Cost Log row.
...
If approved, Gemini updates `functional_validation.md`, feature `README.md`, `docs/knowledge/*` if needed, `pr_description.md`, `status.md`, and Agent Cost Log row.
...
The GitHub Issue is updated only at major milestones, blockers, human decisions, PR creation, or material token/credit budget issues.
