...
Before acting, agents must infer or declare:

- task type;
- scope;
- reasoning need;
- cheapest effective model/mode;
- context budget;
- expected outputs.
...
A feature Issue means a real GitHub Issue created in the remote GitHub repository.

Agents must not replace GitHub Issues with local Markdown files. The workflow does not use and must not create:

```text
docs/issues/
```

Feature coordination must happen through:
...
After the Issue is created, Gemini performs feature preflight before creating the branch. Preflight sources:
...
If Gemini detects any possible overlap or conflict with an active feature, it must block the workflow and ask the human.

After successful preflight, Gemini creates the feature branch and documentation folder in the same initial commit.
...
```text
docs/features/<issue-id>-<short-slug>/
```

## 13. Questions, Decisions and Loops

All questions are centralized in `questions.md`, organized by type. Each question must include an ID, status, source agent, target agent, type, blocking flag, question and answer when resolved.

Important decisions are recorded in `decision_log.md`. Minor decisions are recorded only if they affect observable behavior, acceptance criteria, tests, documentation or user experience.
...
Clarification loops before producing a deliverable do not count. If the Claude ↔ Codex correction limit is reached, Codex escalates to Gemini. If the Codex ↔ Gemini limit is reached, Gemini escalates to the human.

## 14. Testing and Definition of Done

Baseline required checks before PR:
...
If existing tests are failing before implementation starts, Claude must not begin implementation. Every acceptance criterion must be mapped in `test_plan.md` to one or more tests, or explicitly justified as not applicable.

Gemini may create the PR only when all are true:
...
- unit tests green;
- required e2e tests green;
- lint green;
- typecheck green;
- build green;
- security scan green if configured;
